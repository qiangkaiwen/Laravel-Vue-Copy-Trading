			...
			<v-tab-item>
				...
				<v-row>
					<template v-for="(details,index) in clientData.Clients ">
						<app-card v-if="details.tag == 'favourite' || 'recently_added'" :id="index"
							colClasses="col-xl-3 col-lg-3 col-md-4 col-sm-6 col-12" :key="index">
							<v-row class="contact-item-info">
								...
								<v-col cols="12" md="12" class="text-center pt-0">
									...
									<span class="d-block text-ellipse contact-links font-sm"><a
											href="mailto:JerryBRied@jourrapide.com" class="grey--text">{{ details.e_mail
											}}</a></span>
									<span class="d-block contact-links font-sm"><a class="grey--text"
											href="tel:+12075894752">{{ details.phone_number }}</a></span>
				...
				<v-row>
					<template v-for="(details,key) in clientData.Clients">
						<app-card v-if="details.tag == 'favourite'"
							colClasses="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12" :key="key">
							<v-row class="contact-item-info">
								<v-col cols="12" md="12" class="text-center">
									<div class="tab-image">
										<img :src="details.image" />
								...
								<v-col cols="12" md="12" class="text-center pt-0">
									...
									<span class="d-block contact-links font-sm"><a href="tel:+12075894752"
											class="grey--text">{{ details.phone_number }}</a></span>
									<span class="font-sm grey--text">{{ details.country }}</span>
								</v-col>
							</v-row>
							<v-list three-line class="icon">
						...
						<app-card v-if="details.tag == 'recently_added'"
							...
							<v-row class="contact-item-info">
								...
								<v-col cols="12" md="12" class="text-center pt-0">
									...
									<span class="d-block text-ellipse contact-links font-sm"><a class="grey--text"
											href="mailto:JerryBRied@jourrapide.com">{{ details.e_mail }}</a></span>
									<span class="d-block contact-links font-sm"><a class="grey--text"
											href="tel:+12075894752">{{ details.phone_number }}</a></span>
									<span class="font-sm grey--text">{{ details.country }}</span>
								</v-col>
