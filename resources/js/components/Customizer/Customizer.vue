<template>
	<div>
		<v-navigation-drawer fixed v-model="customizerIn" :width="300" :right="!rtlLayout" app temporary
			class="clearfix">
					...
					<vue-perfect-scrollbar class="customizer-scroll-area" :settings="settings">
						<div class="inner-toolbar">
							...
							<div>
								...
								<v-select outlined :items="routerAnimations" label="Select Animation"
									:value="selectedRouterAnimation" @change="changeRouterAnimation($event)"></v-select>
							</div>
						</div>
							...
							<v-list class="mb-5 theme-layouts">
								<v-list-item>
									<v-list-item-content class="py-0">
										<v-checkbox :label="$t('message.darkMode')" color="primary"
											@change="emitDarkMode($event)"></v-checkbox>
									</v-list-item-content>
								</v-list-item>
								<v-list-item v-if="isSidebarAvailable()">
									<v-list-item-content class="py-0">
										<v-checkbox :label="$t('message.collapseSidebar')" color="primary"
											@change="emitCollapseSidebar" :input-value="collapseSidebar"></v-checkbox>
									</v-list-item-content>
								</v-list-item>
								<v-list-item>
									<v-list-item-content class="py-0">
										<v-checkbox :label="$t('message.rtlLayout')" color="primary"
											@change="emitRtlLayout($event)" :input-value="rtlLayout"></v-checkbox>
									</v-list-item-content>
								</v-list-item>
								<v-list-item v-if="isSidebarAvailable()">
									<v-list-item-content class="py-0">
										<v-checkbox :label="$t('message.backgroundImage')" color="primary"
											@change="emitEnableBackgroundImage" :input-value="backgroundImage">
										</v-checkbox>
									</v-list-item-content>
								</v-list-item>
							</v-list>
							<div class="sidebar-images" v-if="backgroundImage">
								<div class="d-inline-block mx-1 mb-2" v-for="backgroundImage in sidebarBackgroundImages"
									...
									<a href="javascript:void(0)" class="img-holder"
										...
										<img :src="backgroundImage.url" height="200" width="75"
											@click="changeBackgroundImage(backgroundImage)" alt="dynamic sidebar images"
